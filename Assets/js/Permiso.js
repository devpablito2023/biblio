...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const  buttons = [{
        ...
        tblPermiso = $('#tblPermiso').DataTable({
        ajax: {
...
function btnEditarPermiso(id) {
    ...
    const url = base_url + "Permiso/editar/" + id;
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById("nombre_permiso").value = res.nombre_permiso;
...
function btnEliminarPermiso(id) {
    Swal.fire({
        ...
        text: "El permiso no se eliminará de forma permanente",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            const url = base_url + "Permiso/eliminar/" + id;
            const http = new XMLHttpRequest();
            http.open("GET", url, true);
            http.send();
            http.onreadystatechange = function () {
...
function registrarPermiso(e) {
    e.preventDefault();
    const nombre_permiso = document.getElementById("nombre_permiso");
    if (nombre_permiso.value == "") {
        alertas('El Nombre del permiso es requerido', 'warning');
    } else {
        const url = base_url + "Permiso/registrar";
...
function btnReingresarPermiso(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const url = base_url + "Permiso/reingresar/" + id;
            const http = new XMLHttpRequest();
            http.open("GET", url, true);
            http.send();
            http.onreadystatechange = function () {
